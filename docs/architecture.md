...
- **`outputMode: "server"`** — produce sia `dist/fisioterapista/browser/` (statico) sia `dist/fisioterapista/server/server.mjs` (Node)
...
`src/assets/mock/*.json` — file statici importati via `HttpClient` o `fetch` lato client, oppure via direct `import` lato server (SSR). Vedi pagina [Mock Data](/mock-data) per la struttura specifica di Studio Fisioterapista.
